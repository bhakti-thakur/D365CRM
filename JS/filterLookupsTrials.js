...
function filterLookup(executionContext) {
    var formContext = executionContext.getFormContext();
    var studentControl = formContext.getControl("ud_student");

    if (!studentControl) {
        console.error("Control 'ud_student' not found on the form.");
        return;
    }

    studentControl.addPreSearch(function () {
        // Only show students whose name starts with 'a'
        var studentFilter = "<filter type='and'>" +
                                "<condition attribute='ud_name' operator='like' value='a%' />" +
                            "</filter>";

        // First parameter: the FetchXML filter
        // Second parameter: the entity logical name — ud_student
        studentControl.addCustomFilter(studentFilter, "ud_student");
      console.log("filter lookup");
    });
}
